...
%% Make ground truth - threshold evals
...
saveEigsImagPart    = zeros(numel(Fgt2),size(Fgt2{1},1));
for ii = 1:numel(Fgt2)
    saveEigsImagPart(ii,:) = imag(eig(Fgt2{ii}));
end

figure();histogram(saveEigsImagPart);title('Imaginary part, model good')
% for ii = 1:numel(Fgt{1})
%     saveEigsImagPartTop(counter,:) = imag(eig(Fgt2{ii}));
%     counter = counter+1;
%     for jj = numel(Fgt{1})+1:numel(Fgt{1})+numel(Fgt{2})
%         disp(eig(Fgt2{jj}))
%         disp(eig(Fgt2{ii}+Fgt2{jj}))
%         counter = counter+2;
%     end
% end
%%
...
inf_opts.lambda_f_decay  = input('lambda_f_decay (e.g., 0.9999):'); %0.996
...
varExplBhv = mean(varExplPerBehaviorTrace,'all','omitnan');
% %%
% figure()
% set(gcf,"Color","white")

...
doSmoothedBehavior = 1;%input('Smooth behavior?');

if doSmoothedBehavior
    smoothingwindow = 20;%input('Sigma for smoothing?');
...
doMedianFilter = 1;%input('Median filter (smoothing)? (1 or 0):');
if doMedianFilter
    medFiltSize = 20;%input('Median filter window size (recommended > 5):');
    dFF = medfilt2(dFF,[1,medFiltSize]);
end

%% rescale data to mode (by neuron) after filtering

doDFOverF = 1;%input('Do dF/F (1) or just dF (0)?:');
...
if doDFOverF
    ...
    softnormterm = 1;%input('Soft norm term (rec. 1):'); %prctile(stdDFF,50);
...
%%
doLB = 1;%input('With Ljung-Box? (1 or 0):');
...
%%
doRescaleToMaxAbs = 0;% input('Rescale to max abs value? (0 or 1):');
...
%%
% Set parameters
inf_opts.nF              = 25;%input('nF (sugg. > 10):'); % 10 not enough - using all
inf_opts.N               = 50;%input('n:'); %scaled down from 100 (zebrafish, whole brain) 

disp('If reset dFF below, make sure to change inf_opts.M');
inf_opts.lambda_val      = 0.05;%input('lambda_val (sugg. 0.1):'); % 0.1 % VARY - tradeoff: approx. SNR - see ratios of traces
inf_opts.lambda_history  = 0.23;%input('lambda_history (sugg. 0.9):'); % 0.9
inf_opts.lambda_b        = 0.05;%input('lambda_b (sugg. 0.01):'); % 0.01 %FIXME: stronger than 0.01 (also other reg params) - 0.1 all disappear, 0.05 only 2 traces
inf_opts.lambda_historyb = 0;
inf_opts.behaviordLDS    = 1;%input('behavior? (0 or 1):');
inf_opts.lambda_behavior = 1;%input('lambda_behavior (sugg. 1.3):');
inf_opts.tol             = 1e-3; % 1e-3
inf_opts.max_iter2       = 50; %500 
inf_opts.max_iters       = 5000;%input('max_iters (sugg. > 2000):');
inf_opts.special         = '';
inf_opts.F_update        = true;
inf_opts.D_update        = true;
% inf_opts.AcrossIndividuals     = false;
% inf_opts.AcrossIndividuinals     = 0; %input('Across individuals? 1 for yes, 0 for no:');
inf_opts.N_ex            = 40;%input('N_ex:');%40;
inf_opts.T_s             = 30;%input('T_s:');%30;
% inf_opts.sampleProportionally = true; %default: prop by trial length
% inf_opts.special         = 'noobs';
inf_opts.solver_type     = 'tfocs';%input('Solver (tfocs or fista):','s');
...
inf_opts.lambda_f        = 0.05;%input('lambda_f:'); % ASC added 7/28'
inf_opts.step_psi        = 10;

% inf_opts.lambda_D        = 0.0005; % 0.01 too strong, D mostly zeros after 500 iterations; decay; 0.0005 good for SG fish
% inf_opts.fewerneurpercol = 1;
% inf_opts.Dnorm           = 'norm'; % default
...
save(sprintf('saveFish_En_BToX%s.mat',dtToStr),'-v7.3')

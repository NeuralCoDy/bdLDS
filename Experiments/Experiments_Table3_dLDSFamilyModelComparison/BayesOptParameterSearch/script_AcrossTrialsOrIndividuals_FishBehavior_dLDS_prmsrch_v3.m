...
inf_opts.behaviordLDS = 0; % no behavior
...
fun = @(x)runDLDSBO_opt3params_zebrafish(dFF,...
    x.lambda_val,x.lambda_history,x.lambda_b,inf_opts);
results = bayesopt(fun,[lambda_val,lambda_history,lambda_b],'Verbose',0,...
    'AcquisitionFunctionName','expected-improvement-plus','OutputFcn',@saveBOPlots);

%%
save(sprintf('dlds_prmsrch_BayesOpt_%s.mat',todaysDate),'results')
%%

...
rng default
lambda_history = optimizableVariable('lambda_history',[0.1 1],'Type','real'); % [0.001 1] --> 0.5536
lambda_behavior = optimizableVariable('lambda_behavior',[0.1 4],'Type','real'); % [0.1 10], real --> 9.9994
fun = @(x)runBehaviorDLDSXBO(dFF,behavior_data,...
    x.lambda_history,x.lambda_behavior,inf_opts);
results = bayesopt(fun,[lambda_history,lambda_behavior],'Verbose',0,...
    'AcquisitionFunctionName','expected-improvement-plus','OutputFcn',@saveBOPlots);

%%
save('bdldsx_prmsrch_BayesOpt_260419.mat','results')
%%

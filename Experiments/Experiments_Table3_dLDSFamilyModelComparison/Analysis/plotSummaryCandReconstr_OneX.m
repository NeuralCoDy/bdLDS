function plotSummaryCandReconstr_OneX(whichData,whichPhi,whichDynCoeffs,whichLatentStates,inf_opts,trialIdentifier,varExplPerNeuron,samplingRate, varargin)
behavior1 = [];
behavior2 = [];
listOfXs  = [];
if nargin > 11
...
if inf_opts.AcrossIndividuals
    for ii = 1:size(whichPhi,1)
        ...
        fig1 = figure();
        subplot(3,1,1)
        x_values = cell2mat(whichDynCoeffs{ii});
        plot(x_values.')
        hold on
        ylabel("c")
        hold off
    ...
    for ii = 1:size(whichLatentStates,1) %in the event of multiple trials
        ...
        x_values_all = whichLatentStates{ii};

        if isempty(listOfXs)
            listOfXs = 1:size(x_values_all,1);
        end

        for whichX = listOfXs %1:size(c_values_all,1) 
            x_values        = x_values_all(whichX,:);

            
            c_values        = whichDynCoeffs{ii};
            reconstrY       = (whichPhi(:,whichX) * x_values).';
            origY           = whichData{ii}.';
            nTimepoints     = size(x_values,2);

            corrC       = corr(x_values.',reconstrY); % one for each neuron
        ...
        %     title({'';'Estimated Y'});
            ...
            cmapMaxDistinct = distinguishable_colors(size(x_values,2));
            % subplot(5,1,4), cla;
            nexttile
            hold on;
            % bottom = min(c_values,[],'all');
            % top = max(c_values,[],'all');
            [row0, ~] = find(abs(x_values) < 0.0001*max(abs(x_values(:)),[],'all')); % find small values
            GC = groupcounts(row0); % tally up small values in each row
            % for jj=size(c_values,1)
            Y = x_values;
            if isempty(GC) | GC~=size(x_values,2) % check: are all the values small? if not, allow to plot
                plot(tVals,Y,'color',cmapMaxDistinct(whichX,:),'DisplayName',num2str(whichX));
            end
            hold on;
            % end
        ...
        %     xlabel('Time (s)');
            ...
            filename = sprintf("%s_%s_X%d.fig", dataIdentifier, trialIdentifier,whichX);
            saveas(fig1,filename);

           
            filename2 = sprintf("%s_%s_X%d.pdf", dataIdentifier, trialIdentifier,whichX);

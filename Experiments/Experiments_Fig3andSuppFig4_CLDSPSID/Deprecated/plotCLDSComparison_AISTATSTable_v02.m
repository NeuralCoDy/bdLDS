...
for howManyC = 1:6
    for whichSeed = 1:10
        ...
        thisMSEbdLDS10 = thisMSEbdLDS10/bslnMSE;
        % thisMSECLDS  = thisMSECLDS/bslnMSE;
        %% store in b-dLDS, CLDS tables (rows: seeds, columns: number of coefficients, i.e., nonzero columns of Phi)
...
hold on
errorbar(1:6,mean(meanMSEbdLDS),std(meanMSEbdLDS)/sqrt(10),'DisplayName','b-dLDS, 1 bhv')
errorbar(1:6,mean(meanMSEbdLDS10),std(meanMSEbdLDS10)/sqrt(10),'DisplayName','b-dLDS, 10 bhv')
errorbar(1:6,mean(meanMSECLDS),std(meanMSECLDS)/sqrt(10),'DisplayName','CLDS, 1 bhv')
% yscale("log")
ylabel('Relative MSE of dynamics matrix A')
xlabel('Number of true nonzero columns of true Phi (dynamics tied to behavior)')
xlim([0 6.5])
legend
hold off
grid off

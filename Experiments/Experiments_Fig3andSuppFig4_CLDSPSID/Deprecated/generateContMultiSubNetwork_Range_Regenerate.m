function [xMatrix, varargout] = generateContMultiSubNetwork_Range_Regenerate(nTp, nG, nSC, nTr, varargin)
...
if nargin > 8;   priorFgt = varargin{5}; % allows you to generate more samples with the same ground truth F
else;            priorFgt = [];   
end
...
if isempty(priorFgt)
    ...
    fSubCirc1 = priorFgt{1};
    fSubCirc2 = priorFgt{2};
end
...
elseif Multiswitch
    if contOpt
        [xMatrix, groundTruthC] = generateRandSwitchSysCont(nTr,nNeurons,...
                               nTimepoints, topRows, bottomRows, contOpt);
    else
        [xMatrix, groundTruthC] = generateRandSwitchSysDisc(nTr, ...
                         nNeurons, nNeuronsGroup1, nNeuronsGroup2, ...
                               nTimepoints, topRows, bottomRows, contOpt);
    end
    if nargout > 1;  varargout{1} = {fSubCirc1, fSubCirc2};     end
    if nargout > 2;  varargout{2} = groundTruthC;     end
end
...
function [xMatrix, groundTruthC] = generateRandSwitchSysDisc(nTr, nNeurons, nNeuronsGroup1, nNeuronsGroup2, nTimepoints, topRows, bottomRows, contOpt)

groundTruthC = cell(nTr,1);
xMatrix = cell(nTr,1);
for ll = 1:nTr
    xMatrix{ll}      = ones(nNeurons,nTimepoints);
    xMatrix{ll}(:,1) = randn(nNeurons,1);
    groundTruthC{ll}  = zeros(numel(topRows)+numel(bottomRows),nTimepoints);
    tNow             = 0;
    while tNow < nTimepoints-1
        ...
        randCoef1 = (0.8+(0.4)*rand);
        randCoef2 = (0.8+(0.4)*rand);
        xMatrix{ll}(1:nNeuronsGroup1,tNow+1)=randCoef1*xMatrix{ll}(1:nNeuronsGroup1,tNow+1)./norm(xMatrix{ll}(1:nNeuronsGroup1,tNow+1));
        xMatrix{ll}(nNeuronsGroup1+(1:nNeuronsGroup2),tNow+1)=randCoef2*(0.8+(0.4)*rand)*xMatrix{ll}(nNeuronsGroup1+(1:nNeuronsGroup2),tNow+1)./norm(xMatrix{ll}(nNeuronsGroup1+(1:nNeuronsGroup2),tNow+1));
        ...
        if selF(1) > 0
            groundTruthC{ll}(selF(1), (tNow+1):(tNow+tJump+1)) = randCoef1; %1;
        end
        if selF(2) > 0
            groundTruthC{ll}(numel(topRows)+selF(2), (tNow+1):(tNow+tJump+1)) = randCoef2; %1;
        end
...
function [xMatrix, groundTruthC] = generateRandSwitchSysCont(nTr, nNeurons, nTimepoints, topRows, bottomRows, contOpt)
...
for ll = 1:nTr
    ...
    while tNow < nTimepoints-1
        ...
        if selF(1) > 0
            groundTruthC{ll}(selF(1), (tNow+1):(tNow+tJump+1)) = 1;
        end
        if selF(1) > 0
            groundTruthC{ll}(selF(1), (tNow+1):(tNow+tJump+1)) = 1;
        end
        ...
        if selF(1) > 0
            groundTruthC{ll}(abs(selF(1)), (tNow+1):(tNow+tJump+1)) = sign(selF(1));
        end
        if selF(2) > 0
            groundTruthC{ll}(numel(topRows)+abs(selF(2)), (tNow+1):(tNow+tJump+1)) = sign(selF(2));
        end
    end
end
end

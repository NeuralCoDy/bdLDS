...
[latentStatesXExtra, FgtNew, groundTruthStatesExtra] = generateContMultiSubNetwork_Range_Regenerate(...
               sizeT,[sizeD/2,sizeD/2],nF,10,contOpt,false,false,true,Fgt);
%%
nBhv = 1;
simulatedPsi = [1 0 0 0 0 0];
...
behaviorDataTest = cellfun(@(x) simulatedPsi*x,justOneGTStateExtra,'UniformOutput',false);
%%
% noiseLevel = input('noise level (default:0):');
% if isempty(noiseLevel)
%     noiseLevel = 0;
% end
dFFTest = statesToObservations(simulatedDmatrix, latentStatesXExtra, noiseLevel);
save('saveFMT_1bhvgraded_forCLDS_TMLRExtra.mat')
%%
% figure(), cla;
% tSel = 1;
...
% inf_opts.T_s             = 100;%input('T_s (e.g., 200):');%200; 
...
%%
% load("saveFMT_1bhvgraded_forCLDS_GOOD_251113.mat")
% inf_opts.N_ex            = 300;%input('N_ex (e.g., 50):');%50;   
% inf_opts.T_s             = 200;%input('T_s (e.g., 200):');%200;
% %%
% dFF2      = dFF(1:40);
% behavior2 = behaviorData(1:40);
% if inf_opts.behaviordLDS
%     [Phi, F, Psi] = bpdndf_dynamics_learning_behavior(dFF2, [], [], [], behavior2, inf_opts);              % Run the dictionary learning algorithm
% else
...
% end
% 
% save('saveFMT_1bhvgraded_forCLDS_TMLR40.mat')
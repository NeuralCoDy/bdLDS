function plotSummaryCandReconstr(whichData,whichPhi,whichDynCoeffs,whichLatentStates,inf_opts,trialIdentifier,varExplPerNeuron,samplingRate, varargin)
...
if inf_opts.AcrossIndividuals
    ...
    %     hold off;
        ...
        for jj=1:size(c_values,1)
            Y = c_values(jj,:);
            if isempty(GC) | GC(jj)~=size(c_values,2) % check: are all the values small? if not, allow to plot
                plot(tVals,Y,'color',cmapMaxDistinct(jj,:),'DisplayName',num2str(jj));
            end
            hold on;
        end
        ...
        filename = sprintf("%s_%s.fig", dataIdentifier, trialIdentifier);
        saveas(fig1,filename);

       
        filename2 = sprintf("%s_%s.pdf", dataIdentifier, trialIdentifier);
